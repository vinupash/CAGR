...
const Gender = ({ navigation, route }) => {
    ...
    const submitData = async () => {
        ...
        try {
            ...
            if (!option) {
                handleErrorMsg()
                setErrorMessage('Plsese select gender');
                return
            }
            ...
            const response = await fetch(BASE_URL + "register/step2", requestOptions);
            const json = await response.json();
            setLoading(false);
            console.log('Gender json --->', json);
            if (json.status == true) {
                alert(json.message)
                handleSuccessMsg()
                setSuccessMessage(json.message)
                navigation.navigate('Occupation', { user_id: user_id, user_mobile_no: user_mobile_no })
            } else {
                handleErrorMsg()
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.bottomSectionPage}>
                <SecondaryBtn
                    btnText='Submit'
                    onPress={() => navigation.navigate('Occupation', { user_id: user_id, user_mobile_no: user_mobile_no })}
                // onPress={submitData}
                />
            </View>
        </SafeAreaView>
