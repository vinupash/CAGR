...
import { IncomeApi, InvesterTypeApi, OccupationApi } from '../../../constants/AllApiCall';
import RadioButtonBox from '../../../components/RadioButtonBox';
const windowHeight = Dimensions.get('window').height;
const windowWidth = Dimensions.get('window').width;

const InvestorType = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const fetchDataAsync = async () => {
            setLoading(true)
            const result = await InvesterTypeApi();
            setLoading(false)
            setData(result.result);
            const isDataList = [...result.result];
            const newArrayList = isDataList.map((item) => {
                return { id: item.fldi_cat_id, value: item.fldv_cat_name, title: item.fldv_cat_name }
            })
    ...
    const submitData = async () => {
        ...
        try {
            ...
            const response = await fetch(BASE_URL + "register/updateinvestor", requestOptions);
            const json = await response.json();

            console.log('InvestorType json --->', json);
            if (json.status == true) {
                alert(json.message)
                handleSuccessMsg()
                setSuccessMessage(json.message)
                navigation.navigate('TaxStatus', { user_id: user_id, user_mobile_no: user_mobile_no })
            } else {
                handleErrorMsg()
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.bottomSectionPage}>
                <SecondaryBtn
                    btnText='Submit'
                    onPress={() => navigation.navigate('TaxStatus', { user_id: user_id, user_mobile_no: user_mobile_no })}
                // onPress={submitData}
                />
            </View>
        </SafeAreaView>
