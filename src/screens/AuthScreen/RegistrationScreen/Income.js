...
import { IncomeApi, OccupationApi } from '../../../constants/AllApiCall';
...
const Income = ({ navigation, route }) => {
    ...
    const submitData = async () => {
        ...
        try {
            ...
            const response = await fetch(BASE_URL + "register/updateincome", requestOptions);
            const json = await response.json();
            setLoading(false);

            console.log('Income json --->', json);
            if (json.status == true) {
                alert(json.message)
                handleSuccessMsg()
