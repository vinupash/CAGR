...
import { OccupationApi } from '../../../constants/AllApiCall';
...
const Occupation = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const fetchDataAsync = async () => {
            setLoading(true)
            const result = await OccupationApi();
            setLoading(false)
            setData(result.result);
            const isDataList = [...result.result];
            const newArrayList = isDataList.map((item) => {
                return { id: item.id, value: item.description, title: item.description }
            })
    ...
    const submitData = async () => {
        ...
        try {
            ...
            if (!option) {
                handleErrorMsg()
                setErrorMessage('Plsese select Occupation');
                return
            }
            ...
            const response = await fetch(BASE_URL + "register/updateoccupation", requestOptions);
            const json = await response.json();
            setLoading(false);

            console.log('Occupation json --->', json);
            if (json.status == true) {
                alert(json.message)
                handleSuccessMsg()
                setSuccessMessage(json.message)
                navigation.navigate('Income', { user_id: user_id, user_mobile_no: user_mobile_no })
            } else {
                handleErrorMsg()
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.bottomSectionPage}>
                <SecondaryBtn
                    btnText='Submit'
                    onPress={() => navigation.navigate('Income', { user_id: user_id, user_mobile_no: user_mobile_no })}
                // onPress={submitData}
                />
            </View>
        </SafeAreaView>
